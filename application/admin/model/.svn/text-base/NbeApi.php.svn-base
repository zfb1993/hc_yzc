...
class NbeApi extends Model
{
    ...
    public function createAccount($data)
    {
        ...
        $dataInfo['chargemouldid'] =$data['chargemouldid'];
        $dataInfo['control'] =$data['control'];
        try{
    ...
    {
        //action=UpdateAccount&account=11113&password=1111&motheraccount=000397
        $dataInfo['action']='UpdateAccount';
        ...
        try{
            ...
            if(!empty($resArr) ){
                ...
                //返回错误
                return array('code'=>-1,'msg'=>'修改密码失败！'.$res);
            }
        }catch (\Exception $e){
            //返回错误
            return array('code'=>-2,'msg'=>'开户失败，资管系统未开启！');
    ...
    public function inOutCash($data)
    {
        ...
        $apiurl = getcofig('WINDOWS_URL');
		oplog('inOutCash--WINDOWS_URL-->'.$apiurl);
		oplog('inOutCash--dataInfo-->'.json_encode($dataInfo));
        try{
            $res=http($apiurl,$dataInfo);
			oplog('后台充值审核NBeApi---->'.serialize($res));
            $resArr=json_decode($res,true);
    ...
    public function getContact($data){
        $dataInfo['instrument']=$data['instrument'];
        $dataInfo['action']='QueryInstrument';
        $apiurl = getcofig('WINDOWS_URL');
        try{
            ...
            $resArr=json_decode($res,true);
            if(!empty($dataInfo['instrument'])){
                $resArr=$resArr[0];
            }
            if(!empty($resArr) && count($resArr)>0){
                return array('code'=>1,'data'=>$resArr,'msg'=>'查询成功！');
            }else{
                //返回错误
