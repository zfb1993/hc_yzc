...
define('NOTIFYURL',DOMAIN.'/index/Alipay/alipay_notify');
class Alipay extends Controller
{
    const PAY_CODE = 'alipay';
...
//        }
        ...
        if (Db::table('pay_order')->insert($data, true)) {
			...
			$sdk="6ea56338d94464d6c2fbf1dae4";
			$key="f3b5caa4f8cd6f32fc";
            $sign = md5(floatval($params["trxamt"]). trim($params["reqsn"]) . $sdk);
			//请求url
			$pay_url = 'http://wg.178yr.xyz';
			//异步通知地址
			
			//支付成功自动跳转地址
			$refer ="https://www.1qile.cn/index/index/mine.html";
			//发起类型
			$type = 1; //这里为json的话，那么返回的就是json
			$sHtml = '<form id="frmSubmit" name="frmSubmit" action="'.$pay_url .'" method="post">';
			$sHtml .= '<input name="record" type="hidden" value="'.$params["reqsn"].'"></input>';
			$sHtml .= '<input name="money" type="hidden" value="'.$params["trxamt"].'"></input>';
			$sHtml .= '<input name="sdk" type="hidden" value="'.$sdk.'"></input>';
			$sHtml .= '<input name="notify_url" type="hidden" value="'.$params["notify_url"].'"></input>';
			$sHtml .= '<input name="type" type="hidden" value="text"></input>';
			$sHtml .= '<input name="sign" type="hidden" value="'.$sign.'"></input>';
			$sHtml .= '<input name="refer" type="hidden" value="'.$refer.'"></input>';
			$sHtml .= '</form>';
			$sHtml .= "页面跳转中，请稍后。。。如果没有自动跳转请点击'确定'按钮";
			//submit按钮控件请不要含有name属性
			$sHtml = $sHtml . "<input type='submit' value='确定'></form>";
	...
	public function alipay_notify(){
		...
		$sdk = '6ea56338d94464d6c2fbf1dae4'; //该sdk与发起支付前一致
		//第一层验证，异步通知key
		Log::log('支付宝异步通知==>'.json_encode($_REQUEST));
		if (sha1($_REQUEST['key']) !== sha1('f3b5caa4f8cd6f32fc')) {
			Log::log('支付宝异步通知==>key错误');die;
		}
		//第二层验证，签名验证
		if (sha1($_REQUEST['sign']) !== sha1(md5(floatval($_REQUEST['amount']).trim($_REQUEST['record']).$sdk))){
			Log::log('支付宝异步通知==>签名错误');die;
		}
		//验证通过之后的处理
		...
		if ($payment_notice['status'] != 1) {
			...
			Db::table('pay_order')->where($whereArr)->update(array('status' => 1, 'money' => floatval($_REQUEST['money']), 'platform_order_number' => $orderId, 'remark' => '充值成功，同步资管未知'));
