...
class Ips extends Controller
{
    ...
    public function create_order()
    {
        ...
        $data['fee'] =intval($params["trxamt"]*0.0045*100)/100;//按千4.5收手续费
...
//同步
	...
	public function tmall_pay(){
        ...
        $billno = $out_trader_no;
        //订单金额(保留2位小数)
        $Amount = number_format(($payment_notice['money']+$payment_notice['fee']), 2, '.', '');
        //支付结果成功返回的商户URL
        $Merchanturl = DOMAIN. '/index/Ips/response';
        //交易返回接口加密方式
        $RetEncodeType = "17";
		...
		if(empty($paymentResult['sign'])){
			...
			if($paymentResult['sign']==$sign_str){
                ...
                if ($payment_notice['status'] != 1) {
                    ...
                    //此处进行业务逻辑处理,给对应的账号加上订单对应可用余额及同步资管账号
                    $userInfo=Db('customer')->where('id',$payment_notice['userid'])->find();
                    $tranAmt =floatval($payment_notice["money"]);;
                    $tranAmt = round($tranAmt, 2);
                    //事物日志控制
                    $total_money=$tranAmt+$userInfo['total_money'];
                    $result=Db('flow_log')->insert([
...
//                    }
	...
	public function sf_return(){
		oplog('同步回调开始-->'.json_encode($_REQUEST));
	   $returnArray = array( // 返回字段
        ...
        if ($sign == $_REQUEST["sign"]) {
            if ($returnArray["returncode"] == "00"&&$returnArray["returncode"]=='SUCCESS') {
				$str = "交易成功！订单号：".$returnArray["orderid"];
				$whereArr['order_number'] = $returnArray["orderid"];
                $payment_notice = Db::table('pay_order')->where($whereArr)->limit(0, 1)->find();
				if(!empty($payment_notice)){
					if ($payment_notice['status'] != 1) {
						oplog('同步回调:>1');
						$orderId = $returnArray['orderid'];
						Db::table('pay_order')->where($whereArr)->update(array('status' => 1, 'platform_order_number' => $returnArray['transaction_id'], 'remark' => '充值成功，同步资管未知'));
						//此处进行业务逻辑处理,给对应的账号加上订单对应可用余额及同步资管账号
						$userInfo=Db('customer')->where('id',$payment_notice['userid'])->find();
						oplog('同步回调:>2-->'. Db('customer')->getLastSql());
						$tranAmt =floatval($payment_notice["money"]);;
						$tranAmt = round($tranAmt, 2);
						//事物日志控制
						$total_money=$tranAmt+$userInfo['total_money'];
						$result=Db('flow_log')->insert([
							...
							'type'=>1,
							'add_time'=>time(),
							'memo'=>'第三方支付线上入金'.$returnArray['orderid']
						]);
						oplog('同步回调:>133--->'.Db('flow_log')->getlastsql());
						if($result){
							oplog('同步回调:>2-->result'.$result);
							$nbeApi=new model\NbeApi();
							$dataInfo['childid']= $userInfo['zid'];
							$dataInfo['cash']= $tranAmt;
							$dataInfo['from']= 'admin input cash';
							$rec= $nbeApi->inOutCash($dataInfo);
							oplog('同步回调:>3-->rec'.json_encode($rec));
							if($rec['code']==1){
								Db('customer')->where('id',$userInfo['id'])->setInc('available_money',$tranAmt);
								Db('customer')->where('id',$userInfo['id'])->setInc('local_money',$tranAmt);
								Db('customer')->where('id',$userInfo['id'])->setInc('total_money',$tranAmt);
						...
						oplog('同步回调:>6');
						oplog('订单重复通知：'  .$returnArray['orderid'] );
						$this->assign('memo', '支付成功');
					}
				}else{
					oplog('订单没找到：'  .json_encode($_REQUEST) );
					$this->assign('memo', $_REQUEST["memberid"]);
				}
                
            }else{
				oplog('订单支付失败：'  .json_encode($_REQUEST) );
				$this->assign('memo', $_REQUEST["memberid"]);
            }
        }else{
			oplog('订单支付失败签名错误：'  .json_encode($_REQUEST) );
			$this->assign('memo', '签名错误');
        }
        return $this->fetch('sf_response');
	}

	public function sf_notify(){
		oplog('异步收到回调:>'.json_encode($_REQUEST));
		$returnArray = array( // 返回字段
            "memberid" => $_REQUEST["memberid"], // 商户ID
            "orderid" =>  $_REQUEST["orderid"], // 订单号
            "amount" =>  $_REQUEST["amount"], // 交易金额
        ...
        if ($sign == $_REQUEST["sign"]) {
            if ($_REQUEST["returncode"] == "00") {
				oplog('异步收到回调---验签正确--开始处理');
					$str = "交易成功！订单号：".$_REQUEST["orderid"];
					$whereArr['order_number'] = $_REQUEST['orderid'];
					$payment_notice = Db::table('pay_order')->where($whereArr)->limit(0, 1)->find();
					if ($payment_notice['status'] != 1) {
						...
						$result=Db('flow_log')->insert([
							...
							'type'=>1,
							'add_time'=>time(),
							'memo'=>'第三方支付线上入金'.$_REQUEST['orderid']
						]);
					if($result){
						$nbeApi=new model\NbeApi();
						$dataInfo['childid']= $userInfo['zid'];
                    ...
                    oplog('订单重复通知：'  .$_REQUEST['orderid'] );
                     echo "OK";//支付成功
                }
            }else{
				oplog('异步通知，订单支付失败-->'.$_REQUEST['orderid']);
            }
        }else{
			oplog('异步通知，订单签名核对错误-->'.$_REQUEST['orderid']);
        }
	}
