...
class Agent extends Common
{
  ...
  public function insManage()
  {
    if(Request()->isPost()){
		...
		$count=count($agent);
		if($count){
			$list=$this->pagedown($agent,$pre,$limit);
			return $result = ['code'=>0,'msg'=>'获取成功','count'=>$count,'data'=>$list];
		}else{
			return $result = ['code'=>-1,'msg'=>'暂无所有下级代理数据...'];
		}
    }
    return $this->fetch();
  }
  ...
  public function agentInfo()
  {
    if(Request()->isPost()){
		...
		$zNodes=[];
		$newArr=[];
		$curAgent=Db('agentor')->field('id,aid,user_name,true_name')->where('id',$aid)->find();
		if($curAgent){
			$i=0;
			$newArr['id']=$i+1; //顶层 1
			$newArr['pId']=$i; // 0
			$newArr['name']=$curAgent['true_name'].'-'.$curAgent['user_name'];
			$newArr['open']=true;
			array_push($zNodes,$newArr);
			//dump($curAgent['aid']);die;
			//判断当前代理的类别
			switch($curAgent['aid']){
				case 1:
					//所有二级代理
					$two_agent=$this->getSubAgentor($curAgent['id']);
					if($two_agent){
						foreach ($two_agent as $j => $value) {
							$newArr['id']=($i+1)*10+($j+1); //2层
							$newArr['pId']=$i+1; // 1
							$newArr['name']=$two_agent[$j]['true_name'].'-'.$two_agent[$j]['user_name'];
							$newArr['open']=true;
							array_push($zNodes,$newArr);
							//所有三级代理
							$three_agent=$this->getSubAgentor($two_agent[$j]['id']);
							if($three_agent){
								foreach ($three_agent as $k => $value) {
									$newArr['id']=($i+1)*100+($j+1)*10+($k+1); //3层
									$newArr['pId']=($i+1)*10+($j+1); // 11
									$newArr['name']=$three_agent[$k]['true_name'].'-'.$three_agent[$k]['user_name'];
									$newArr['open']=true;
									array_push($zNodes,$newArr);
									//所有四级代理
									$four_agent=$this->getSubAgentor($three_agent[$k]['id']);
									if($four_agent){
										foreach ($four_agent as $m => $value) {
											$newArr['id']=($i+1)*1000+($j+1)*100+($k+1)*10+($m+1); //4层
											$newArr['pId']=($i+1)*100+($j+1)*10+($k+1); // 111
											$newArr['name']=$four_agent[$m]['true_name'].'-'.$four_agent[$m]['user_name'];
											$newArr['open']=true;
											array_push($zNodes,$newArr);
										}
									}
				...
				case 2:
					//所有三级代理
					$three_agent=$this->getSubAgentor($curAgent['id']);
					if($three_agent){
						foreach ($three_agent as $j => $value) {
							$newArr['id']=($i+1)*10+($j+1); //2层
							$newArr['pId']=$i+1; // 1
							$newArr['name']=$three_agent[$j]['true_name'].'-'.$three_agent[$j]['user_name'];
							$newArr['open']=true;
							array_push($zNodes,$newArr);
							//所有四级代理
							$four_agent=$this->getSubAgentor($three_agent[$j]['id']);
							if($four_agent){
								foreach ($four_agent as $k => $value) {
									$newArr['id']=($i+1)*100+($j+1)*10+($k+1); //3层
									$newArr['pId']=($i+1)*10+($j+1); // 11
									$newArr['name']=$four_agent[$k]['true_name'].'-'.$four_agent[$k]['user_name'];
									$newArr['open']=true;
									array_push($zNodes,$newArr);
								}
							}
				...
				case 3:
					//所有四级代理
					$four_agent=$this->getSubAgentor($curAgent['id']);
					if($four_agent){
						foreach ($four_agent as $j => $value) {
							$newArr['id']=($i+1)*10+($j+1); //2层
							$newArr['pId']=$i+1; // 1
							$newArr['name']=$four_agent[$j]['true_name'].'-'.$four_agent[$j]['user_name'];
							$newArr['open']=true;
							array_push($zNodes,$newArr);
						}
					}
  ...
  public function edit_insManage()
  {
    if(Request()->isPost()){
	 ...
	 // $fk_id=input('fk_id');
      ...
      if($agent){
        $res=Db('agentor')->where('id',$id)->update(['user_pwd'=>$user_pwd]);
        if($res){
  ...
  public function register()
  {
    ...
    if(Request()->isPost()){
	  ...
	  $agent_config=Db('agent_config')->where('id',$acid)->find();
      if($agent){
        $data=[
		  'aid'=>$agent_config['id'],
          'tid'=>$agent['tid'],
  ...
  {
	$uid=session('aid');
    $agent=Db('agentor')->field('id,true_name')->where('id',$uid)->find();
    if($agent){
      return json($agent);
    }
  }
  ...
  {
	$uid=session('aid');
    $agent=Db('agentor')->where('id',$uid)->find();
    if($agent){
      return json($agent);
    }
  }
  ...
  //获取代理用户数据
  private function getAgentorData(){
    $res=Db('agentor')->where('pid',session('aid'))->where('status','IN',[0,1])->select();
    if($res){
      foreach ($res as $k => $v) {
        switch ($res[$k]['status']) {
          case 1:
