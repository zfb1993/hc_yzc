    ...
    public function accountInfoLog()
  {
    if(Request()->isPost() || !empty(input('export'))){
        ...
        if(!empty($ins_name)){
			...
			$agent=Db('agentor')->where('id',session('aid'))->field('id')->select();
			$aid=[];
			foreach ($agent as $key => $value) {
				if(in_array($agent[$key]['id'],$zidArr)){
					array_push($aid, $agent[$key]['id']);
...
//			print_r($whereArr);
			...
			$count1=Db::connect(config('db_trade'))->name('trader_close_log')->where($whereArr)->count();
			$list1=Db::connect(config('db_trade'))->name('trader_close_log')
				->where($whereArr)
				->limit($pre/2,$limit/2)
				->order('TradingDay','DESC')
				->select();
			$whereArr['Status']=array('in',array('3','4'));
			$count2=Db::connect(config('db_trade'))->name('trader')->where($whereArr)->count();
			$list2=Db::connect(config('db_trade'))->name('trader')
				->where($whereArr)
				->limit($pre/2,$limit/2)
				->order('TradingDay','DESC')
				->select();
			...
			if(!empty($list)){
				foreach($list as $key=>$value){
					...
					if($list[$key]['CombOffsetFlag']==="0"){
						...
						$list[$key]['OpenVolume']=$list[$key]['CloseVolume'];
						$list[$key]['OpenBond'] = 0;
						$list[$key]['OrderSysID'] = 0;
					}
